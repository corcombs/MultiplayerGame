...
function Sprite(scene, imageFile, width, height){ 
  ...
  this.checkBounds = function(){
	...
	this.imgAngle = this.moveAngle;
      ...
      if (this.x+this.width/2 > rightBorder){
      offRight = true;
    }
    
    if (this.x-this.width/2 < leftBorder){
      offLeft = true;
    }
    
    if (this.y +this.height/2 > bottomBorder){
      offBottom = true;
    }
    ...
    if (this.y -this.height/2< 0){
      ...
      if (offRight){
  this.x = rightBorder-this.width/2;
      } // end if
  
      if (offBottom){
  this.y = bottomBorder-this.height/2;
      } // end if
  
      if (offLeft){
  this.x = leftBorder+this.width/2;
      } // end if
  
      if (offTop){
  this.y = topBorder+this.height/2;
      }
...
function Scene(){
    ...
    var element =document.getElementById("sceneGoesHere");
